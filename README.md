...
- 오직 언마운트 될 때만 뒷정리 함수를 호출하고 싶다면 **useEffect** 함수에 빈 배열을 넣으면 된다.
...
}, [value]); // 배열 안의 value를 넣고 빼고의 차이점.... 최적화
```
...
- 커피숍에 비유 해볼께요
- 액션(action)타입 : 커피숍의 메뉴 (아메리카노, 라떼, 카푸치노, 아이스티 ...)
- 액션생성 : 주문서 작성 (아메리카노 하나랑 라떼 하나 주세요)
  - 페이로드 : 주문서 작성 (아메리카노는 **샷추가**, 라떼 우유는 **두유**로 바꿔주세요)
- 디스패치(dispatch) : 주문하기
- 리듀서(Reducer) : 주문 받은걸 만들고 가공해서 손님에게 내 준다.
