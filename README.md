...
- 최상위 컴포넌트에서 여러 컴포넌트를 거쳐 props 로 원하는 상태와 함수를 전달 했지만
- Context API를 사용하면 Context 를 만들어서 원하는 값을 받아와서 사용할 수 있다.
...
- 색상을 props로 받아오는 것이 아니라 colorContext 안에 들어있는 Consumer 라는 컴포넌트가 내장되어있다. Consumer 를 통해 색상을 조회한다.
...
export const colorProvider = ({ chlidren }) => {
  ...
  return (
    <colorContext.Provider value={value}>{chlidren}</colorContext.Provider>
  );
};
